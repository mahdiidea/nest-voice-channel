<template>
  <v-row justify="center" align="center" class="fill-height bg">
    <v-col cols="12" sm="10" md="8" lg="6" xl="4">
      <v-expansion-panels v-model="expansion" mandatory>

        <v-expansion-panel>
          <v-expansion-panel-header
            v-text="$t('text.login_to_your_account')"
          />
          ...
          <v-expansion-panel-content class="content">
            <form-builder
              ...
              elevation="0"
              :outlined-form="false"
              :on-success="onLoggedIn"
              primary-key-icon="mdi-check"
              :primary-key-text="$t('text.login')"
              rounded
              uri="auth"
              :fields="login.fields"
              :object="login.object"
...
export default {
  ...
  created() {
    this.login.fields = [
      {
        name: 'email',
        label: this.$t('text.email'),
        type: 'email',
        icon: 'mdi-email',
      },
      {
        name: 'password',
        label: this.$t('text.password'),
        type: 'password',
        icon: 'mdi-lock',
      },
    ];
